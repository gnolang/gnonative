...
EXAMPLES_DIR := $(make_dir)/examples/react-native
...
ifndef APP_NAME
	...
	cd $(EXAMPLES_DIR) && yarn create expo $(APP_NAME) --template expo-template-blank-typescript
	@echo "Creating ios and android folders"
	cd $(EXAMPLES_DIR)/$(APP_NAME) && yarn expo prebuild
	@echo "Installing yarn dependencies"
	cd $(EXAMPLES_DIR)/$(APP_NAME) && yarn add ${YARN_BASIC_DEPENDENCIES}

# create folders (api, grpc, hooks)
	@mkdir -p $(EXAMPLES_DIR)/$(APP_NAME)/src/api
	@mkdir -p $(EXAMPLES_DIR)/$(APP_NAME)/src/grpc
	@mkdir -p $(EXAMPLES_DIR)/$(APP_NAME)/src/hooks
# copy the essential files
	@cp -r $(react_native_dir)/src/api $(EXAMPLES_DIR)/$(APP_NAME)/src
	@cp -r ./$(TEMPLATE_PROJECT)/src/grpc $(EXAMPLES_DIR)/$(APP_NAME)/src
	@cp -r ./$(TEMPLATE_PROJECT)/src/hooks $(EXAMPLES_DIR)/$(APP_NAME)/src
	@cp -r ./$(TEMPLATE_PROJECT)/src/native_modules $(EXAMPLES_DIR)/$(APP_NAME)/src
	@cp -r ./$(TEMPLATE_PROJECT)/Makefile $(EXAMPLES_DIR)/$(APP_NAME)
	@cp -r ./$(TEMPLATE_PROJECT)/android/.gitignore $(EXAMPLES_DIR)/$(APP_NAME)/android
	@cp -r ./$(TEMPLATE_PROJECT)/ios/.gitignore $(EXAMPLES_DIR)/$(APP_NAME)/ios
	@cp ./templates/tsconfig.json $(EXAMPLES_DIR)/$(APP_NAME)/tsconfig.json
	@cp ./templates/App.tsx $(EXAMPLES_DIR)/$(APP_NAME)/App.tsx
	$(MAKE) add-app-json-entry
# copy ios Sources
	@mkdir -p $(EXAMPLES_DIR)/$(APP_NAME)/ios/$(APP_NAME)/Sources
	@cp -r $(react_native_dir)/ios/$(TEMPLATE_PROJECT)/Sources $(EXAMPLES_DIR)/$(APP_NAME)/ios/$(APP_NAME)/
	@cp $(react_native_dir)/ios/$(TEMPLATE_PROJECT)/$(TEMPLATE_PROJECT)-Bridging-Header.h $(EXAMPLES_DIR)/$(APP_NAME)/ios/$(APP_NAME)/$(APP_NAME)-Bridging-Header.h
	@cp -r $(react_native_dir)/ios/Sources $(EXAMPLES_DIR)/$(APP_NAME)/ios/

# build GnoCore.xcframework for the new app
	@echo "Building GnoCore.xcframework for the new app"
	@echo $(EXAMPLES_DIR)/$(APP_NAME)
	$(MAKE) build.ios TEMPLATE_PROJECT=$(APP_NAME) react_native_dir=$(EXAMPLES_DIR)/$(APP_NAME)

# copy ios project.pbxproj from gnoboard
	$(MAKE) copy-ios-project-pbxproj


JSON_FILE := $(EXAMPLES_DIR)/$(APP_NAME)/app.json
...
copy-ios-project-pbxproj:
	...
	@cp $(react_native_dir)/ios/$(TEMPLATE_PROJECT).xcodeproj/project.pbxproj $(EXAMPLES_DIR)/$(APP_NAME)/ios/$(APP_NAME).xcodeproj/project.pbxproj
	@cd $(EXAMPLES_DIR)/$(APP_NAME)/ios/$(APP_NAME).xcodeproj
	@sed -i.pbxproj 's/gnoboard/$(APP_NAME)/g' $(EXAMPLES_DIR)/$(APP_NAME)/ios/$(APP_NAME).xcodeproj/project.pbxproj
